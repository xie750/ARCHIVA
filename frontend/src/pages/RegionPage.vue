  ...
  <div class="region-page">
    ...
    <main class="container region-content">
      ...
      <section v-for="city in cities" :key="city.city" class="city-section">
        ...
        <div class="city-grid"><RouterLink v-for="building in city.items" :key="building.id" :to="`/buildings/${building.id}`" class="city-card"><div class="city-card-art" :class="[building.coverClass, building.id]"><img v-if="building.image" class="city-card-img" :src="building.image.src" :alt="building.image.alt" :style="{ objectPosition: building.image.position ?? '50% 50%' }" /><template v-else><span>REFERENCE IMAGE / 建筑资料图</span><i class="tower-silhouette" aria-hidden="true" /></template><span v-if="building.image" class="image-credit">{{ building.image.credit }}</span></div><div class="city-card-body"><span>{{ building.era }} · {{ building.location }}</span><strong>{{ building.name }}</strong><p>{{ building.summary }}</p><ArrowRight :size="16" /></div></RouterLink></div>
      </section>
    </main>
  </div>
</template>
