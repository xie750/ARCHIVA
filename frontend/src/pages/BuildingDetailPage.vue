...
<template>
  <div>
    <section class="detail-cover" :class="[building.coverClass, building.id]"><div class="container"><div class="detail-cover-copy"><span class="eyebrow">{{ building.category }} / {{ building.era }}</span><span class="detail-cover-status">FIELD IMAGE / 待接入授权实景</span><h1>{{ building.name }}</h1><div class="pinyin">{{ building.pinyin }}</div><div class="detail-meta"><span>{{ building.location }}</span><span>·</span><span>{{ building.period }}</span><span>·</span><span>{{ building.status }}</span></div></div></div></section>
