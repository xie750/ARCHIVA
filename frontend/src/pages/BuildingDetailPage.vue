...
const activeProvider = computed(() => selectedPoint.value?.provider ?? (activeEmbedUrl.value ? 'sketchfab' : activeManifestUrl.value || activeAssetUrl.value ? 'local-glb' : building.value.model.provider ?? 'pending'))
const activeAssetStatus = computed(() => selectedPoint.value?.assetStatus ?? building.value.model.assetStatus ?? (activeProvider.value === 'pending' ? 'pending' : 'matched'))
const activeSourceUrl = computed(() => selectedPoint.value?.sourceUrl ?? building.value.model.sourceUrl ?? activeEmbedUrl.value)
const activeCredit = computed(() => selectedPoint.value?.credit ?? building.value.model.credit)
const activeViewerHint = computed(() => activeProvider.value === 'pending' ? '3D 模型正在匹配、权属核验和资产发布中' : '拖动旋转 · 滚轮缩放 · 双击聚焦')
...
function enterPoint(point: ScenicPoint) {
...
<template>
  <div>
    <section class="detail-cover" :class="[building.coverClass, building.id]"><img v-if="building.image" class="detail-cover-image" :src="building.image.src" :alt="building.image.alt" :style="{ objectPosition: building.image.position ?? '50% 50%' }" /><div class="container"><div class="detail-cover-copy"><span class="eyebrow">{{ building.category }} / {{ building.era }}</span><span class="detail-cover-status">{{ building.image ? 'REAL IMAGE / 公开授权实景' : 'FIELD IMAGE / 待接入授权实景' }}</span><h1>{{ building.name }}</h1><div class="pinyin">{{ building.pinyin }}</div><div class="detail-meta"><span>{{ building.location }}</span><span>·</span><span>{{ building.period }}</span><span>·</span><span>{{ building.status }}</span></div></div></div><a v-if="building.image" class="detail-image-credit" :href="building.image.sourceUrl" target="_blank" rel="noreferrer">{{ building.image.credit }} · {{ building.image.license }}</a></section>
    <div class="container detail-layout" :class="{ 'model-active': viewMode === 'model' }">
      ...
      <aside class="detail-aside">
        ...
        <div v-else class="viewer-card"><div v-if="hasOverview" class="model-back"><button type="button" @click="returnToOverview">← 返回{{ isScenic ? '景区' : '位置' }}总览</button><span v-if="selectedPoint">{{ selectedPoint.name }} · {{ selectedPoint.status }}</span></div><span class="viewer-label">INTERACTIVE MODEL / {{ building.model.version }}</span><BuildingViewer :key="selectedPoint?.id ?? building.id" :kind="selectedPoint?.modelKind ?? building.model.kind" :variant="activeVariant" :title="selectedPoint?.name ?? building.name" :asset-url="activeAssetUrl" :manifest-url="activeManifestUrl" :embed-url="activeEmbedUrl" :model-provider="activeProvider" :asset-status="activeAssetStatus" :asset-source-url="activeSourceUrl" :asset-credit="activeCredit" /><span class="viewer-note">{{ activeViewerHint }}<br>{{ activeSource ?? '当前 3D 资产正在接入，完成权属与模型核验后开放。' }}</span><a v-if="activeSourceUrl && activeProvider !== 'pending'" class="viewer-source-link" :href="activeSourceUrl" target="_blank" rel="noreferrer">在原平台打开模型 ↗</a></div>
        <div class="source-card"><h3>资料与模型</h3><ul><li><span class="source-type">MODEL</span><br>版本 {{ building.model.version }} · {{ building.model.precision }}</li><li><span class="source-type">3D STATUS</span><br>{{ activeProvider === 'pending' ? '模型接入中' : activeProvider === 'sketchfab' ? '第三方平台嵌入' : activeProvider === 'local-glb' ? '本地 GLB 资产' : activeProvider }} · {{ activeAssetStatus }}</li><li v-if="building.image"><span class="source-type">PHOTO</span><br><a :href="building.image.sourceUrl" target="_blank" rel="noreferrer" style="color:#786b5d;text-decoration:underline">{{ building.image.credit }} · {{ building.image.license }}</a></li><li v-if="building.model.manifestUrl"><span class="source-type">LOD</span><br>按设备自动选择 low / medium / high</li><li v-if="activeProvider === 'sketchfab'"><span class="source-type">PLATFORM</span><br><a v-if="activeSourceUrl" :href="activeSourceUrl" target="_blank" rel="noreferrer" style="color:#786b5d;text-decoration:underline">{{ activeCredit ?? 'Sketchfab 原平台交互查看器' }}</a><span v-else>Sketchfab 原平台交互查看器</span></li><li v-else-if="activeProvider !== 'pending'"><span class="source-type">COMPONENTS</span><br>{{ building.model.nodes }} 个核心构件组</li><li v-if="building.model.license"><span class="source-type">RIGHTS</span><br>{{ building.model.license }}</li><li><span class="source-type">LOCATION</span><br>{{ building.location }} · {{ building.coordinates[1].toFixed(4) }}°N, {{ building.coordinates[0].toFixed(4) }}°E</li><li v-for="source in building.sources ?? []" :key="source.label"><span class="source-type">{{ source.kind }}</span><br><a v-if="source.href" :href="source.href" target="_blank" rel="noreferrer" style="color:#786b5d;text-decoration:underline">{{ source.label }}</a><span v-else>{{ source.label }}</span></li></ul></div>
      </aside>
    </div>
