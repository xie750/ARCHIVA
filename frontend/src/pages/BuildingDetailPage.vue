  ...
  <div>
    ...
    <div class="container detail-layout" :class="{ 'model-active': viewMode === 'model' }">
      ...
      <aside class="detail-aside">
        ...
        <div class="source-card"><h3>资料与模型</h3><ul><li><span class="source-type">MODEL</span><br>版本 {{ building.model.version }} · {{ building.model.precision }}</li><li v-if="building.image"><span class="source-type">PHOTO</span><br><a :href="building.image.sourceUrl" target="_blank" rel="noreferrer" style="color:#786b5d;text-decoration:underline">{{ building.image.credit }} · {{ building.image.license }}</a></li><li v-if="building.model.manifestUrl"><span class="source-type">LOD</span><br>按设备自动选择 low / medium / high</li><li v-if="building.model.embedUrl"><span class="source-type">PLATFORM</span><br>Sketchfab 原平台交互查看器</li><li v-else><span class="source-type">COMPONENTS</span><br>{{ building.model.nodes }} 个核心构件组</li><li v-if="building.model.license"><span class="source-type">RIGHTS</span><br>{{ building.model.license }}</li><li><span class="source-type">LOCATION</span><br>{{ building.location }} · {{ building.coordinates[1].toFixed(4) }}°N, {{ building.coordinates[0].toFixed(4) }}°E</li><li v-for="source in building.sources ?? []" :key="source.label"><span class="source-type">{{ source.kind }}</span><br><a v-if="source.href" :href="source.href" target="_blank" rel="noreferrer" style="color:#786b5d;text-decoration:underline">{{ source.label }}</a><span v-else>{{ source.label }}</span></li></ul></div>
      </aside>
    </div>
