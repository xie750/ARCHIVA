...
<template>
  <div class="container">
    ...
    <div class="building-grid"><RouterLink v-for="building in explore.filteredBuildings" :key="building.id" :to="`/buildings/${building.id}`" class="building-card"><div class="building-cover" :class="[building.coverClass, building.id]" role="img" :aria-label="building.image?.alt ?? `${building.name} 建筑资料图`"><img v-if="building.image" class="building-cover-img" :src="building.image.src" :alt="building.image.alt" :style="{ objectPosition: building.image.position ?? '50% 50%' }" /><div v-else class="building-cover-placeholder"><span>REFERENCE IMAGE</span><i class="tower-silhouette" aria-hidden="true" /><strong>建筑资料图</strong></div><span v-if="building.image" class="image-credit">{{ building.image.credit }}</span></div><div class="building-body"><div class="card-meta"><span>{{ building.location }}</span><span>{{ building.era }}</span></div><h3>{{ building.name }}</h3><div class="card-category">{{ building.category }}</div><p>{{ building.summary }}</p><div class="card-footer"><span class="card-tag">{{ building.status }}</span><ArrowUpRight :size="16" color="#a73e32" /></div></div></RouterLink><div v-if="!explore.filteredBuildings.length" class="empty-state">没有找到匹配的建筑样本，请更换关键词或分类。</div></div>
  </div>
</template>
