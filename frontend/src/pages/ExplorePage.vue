...
<template>
  <div class="container">
    <section class="explore-header"><span class="eyebrow">FIELD GUIDE / CHINA</span><h1>建筑探索</h1><p>从城市、年代和建筑类型进入中国建筑的空间档案。当前首发展区为河南篇，后续将持续扩展至更多省份。</p></section>
    ...
    <div class="map-panel"><div class="map-copy"><span class="eyebrow">GEOGRAPHY / 34°N—35°N</span><strong>建筑分布在<br>首发展区的山河之间</strong><span>洛阳 · 登封 · 开封</span></div><div class="map-shape"><i v-for="building in explore.filteredBuildings" :key="building.id" class="map-dot" :style="{ left: `${(building.coordinates[0] - 112.2) * 330 + 8}px`, top: `${(35.0 - building.coordinates[1]) * 170 + 15}px` }" :title="building.name" /></div><div class="map-legend"><span><i class="legend-dot" /> 已收录建筑</span><span>河南篇 · 第一章</span></div></div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin:20px 0 18px;color:#968b7e;font-size:11px"><span>{{ explore.filteredBuildings.length }} 座建筑样本</span><span style="display:flex;align-items:center;gap:5px"><MapPin :size="13" /> 河南篇 · 第一章</span></div>
